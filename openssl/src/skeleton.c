...
#else
  synch_sync(s); 
  asm volatile(".rept 0x166; nop; .endr;");
#endif
}
...
int main(int argc, char ** argv) {
  ...
  for(i = 0; i < NEXPTS; i++)
  {
    ...
    for(j = 0; j < NSAMPLES; j++) {
      ...
      *(unsigned *)(pt + 4) = 0xacacf0ac + (0x400 * i);
      asm(NOP4096);
  ...
  FILE *fp_attacker_times, *fp_attacker_zero, *fp_attacker_ones;
  /* Output stats */
  if(proc == PMC_PROC){
    for(i = 0; i < NEXPTS; i++){
      ...
      for(j = 0; j < NSAMPLES; j++) {
        ...
        if(ctrs[i][j][0] == 8)
          ct8++;
        else if(ctrs[i][j][0] == 16)
          ct16++;
      }
      printf("8-> %u, 16-> %u, Ratio: %f\n", ct8, ct16, ((float)ct8)/ct16);
    }
  } 
  if(proc == 'a') {
    ...
    fp_attacker_zero = fopen("zero.csv", "w+");
    fp_attacker_ones = fopen("ones.csv", "w+");
    for(i = 0; i < NEXPTS; i++){
      FILE *fp = (i % 2)? fp_attacker_ones: fp_attacker_zero;
      for(j = 0; j < NSAMPLES; j++)
        fprintf(fp, "%d\n", attacker_times[i][j]);
    }
    fclose(fp_attacker_zero);
    fclose(fp_attacker_ones);
  }
}